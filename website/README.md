...
The pages show current evidence and suite membership, then organize the current definition as supplied input, executor when applicable, runner checks, case decision, and latest execution. A case with fixture files labels its input **Prompt and starting repository** and explains that the fixture is the initial state copied into a disposable workspace, not an evaluation result. A case without fixture files labels the input **Prompt**.
