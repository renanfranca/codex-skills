...
The pages show current evidence and suite membership, then organize the current definition as supplied input, executor when applicable, runner checks, and case decision. Archived executions remain available through operation history. A case with fixture files labels its input **Prompt and starting repository** and explains that the fixture is the initial state copied into a disposable workspace, not an evaluation result. A case without fixture files labels the input **Prompt**.

**Runner checks** separate checks applied automatically by the runner from requirements declared by the case. Automatic checks identify the executor exit code, structured executor response, and evaluated skill integrity when applicable. Declared requirements identify required paths, protected changed paths, workspace check commands, and the expected exit for each command. A nonzero expected command exit is explicitly described as that command's success condition. Oracle commands are repository controlled code kept outside the executor visible workspace and run as separate processes against the resulting workspace. Oracle and judge sections remain visible as **Not used in this case** when disabled, although disabled mechanisms are omitted from the definition flow.

Judge configuration is translated into its effect: `no_action_acceptable: true` permits a response without workspace changes when the declared criteria are satisfied, while `false` means the absence of changes is not sufficient by itself. The case decision follows the runner path: deterministic cases show `PASS` and `FAIL`; executor cases without a judge add `ERROR`; executor cases with a judge also add `INCONCLUSIVE`. `SKIPPED` is only a possible judge state after an earlier failure, not a final observation result.

The catalog explains mechanisms and declarations available in the canonical case contract. It does not invent a case specific purpose, expected behavior, fixture role beyond its starting state, concrete oracle assertions, mechanism weight, or decisive mechanism when the canonical contract does not declare those facts.
...
An **API reference estimate** applies an archived dated API price table to recorded usage. It is never an observed charge, ChatGPT billing statement, or invoice. On report pages, a recorded estimate starts collapsed and expands on request to show its archived status, currency, amount, base rate amount, calculation tokens, prices, components, long context assessment, and limitations. A missing estimate remains visible as `Not recorded` without an empty disclosure. Monetary values retain their recorded decimal value without scientific notation. When request scoped long context pricing cannot be audited from event scope, the page labels the base rate amount as reference only and states that the exact estimate is unavailable.
