...
python3 <skill>/scripts/workflow_state.py \
  --state .agent/tmp/<slug>.dsh-workflow.json \
  init --slug <slug> --plan .agent/tmp/<slug>.md \
  --repo <absolute-repo> --branch <branch> --base <base> --base-sha "$base_sha" --provider <configured-provider>
python3 <skill>/scripts/workflow_state.py \
  --state .agent/tmp/<slug>.dsh-workflow.json show
```
...
```text
register-session --role <role> --session-id <id> --provider <provider> --model <model> --effort <configured-effort>
...
Schema v4 requires the six configured specialist registrations before the first
`implementing` transition. Each records the actual provider, model and reasoning
effort accepted by DSH. Every ID must be non-empty and reused; model changes
cannot replace a registered session ID. Role and lease rules are unchanged.
Legacy versions retain their historical model validation.

`migrate-models --selections <JSON> --reason <reason>` explicitly migrates a
six-specialist v3 ledger to v4. JSON is keyed by role, with `session_id`,
`provider`, `model` and `effort`. All six IDs must match existing registrations.
The operation preserves phase, history, leases, commits, gates and evidence;
`model_changes` records before/after selections. Repeating the same migration
is idempotent. Schema-v4 model updates also record before/after selections.
Reading a legacy ledger never migrates it.
...
The schema-v3 and schema-v4 forward phases are:
...
Use `final-committing` only when post-review work changed the checkout; the direct `habit-rechecking -> final-validating` path means no delivery delta. Any correction returns to `implementing` with a non-empty Coordinator note and repeats all downstream gates. Schemas v3 and v4 have no corrective shortcut from final validation to Habit or structural review.
...
## Ledger separation
DSH ledgers are named `<slug>.dsh-workflow.json`, so the original Codex workflow does not adopt them. Native registrations use `sessions[role].session_id`. Original Codex ledgers are not migrated or rewritten. Transport recovery metadata is stored in `<slug>.dsh-orchestration.json` and records the Coordinator session, reserved IDs, assignments and request IDs.
