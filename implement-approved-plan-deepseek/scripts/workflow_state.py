...
SPECIALISTS_BY_SCHEMA = {
  1: {
    "implementer": ("deepseek/deepseek-v4.1-flash", "high"),
    "committer": ("deepseek/deepseek-v4.1-flash", "high"),
    "validator": ("deepseek/deepseek-v4.1-flash", "high"),
    "habit-curator": ("deepseek/deepseek-v4.1-flash", "high"),
  ...
  3: {
    ...
    "validator": ("deepseek/deepseek-v4.1-flash", "high"),
    "habit-curator": ("deepseek/deepseek-v4.1-flash", "high"),
    "mutation-analyst": ("deepseek/deepseek-v4.1-flash", "high"),
    "structural-reviewer": ("deepseek/deepseek-v4.1-flash", "high"),
  },
}
...
def configured_session_is_valid(chat):
  return (
    fields_match(chat, {"session_id": str, "provider": str, "model": str, "effort": str})
    and all(chat[key].strip() for key in ("session_id", "provider", "model"))
    and chat["effort"] in ("off", "low", "high", "max")
  )
...
def nested_records_are_valid(ledger):
  ...
  chats_valid = all(
    role in specialists
    and fields_match(chat, {"session_id": str, "model": str, "effort": str})
    and (
      configured_session_is_valid(chat)
      if ledger["schema_version"] == 4
      else (chat["model"], chat["effort"]) == specialists[role]
    )
    for role, chat in ledger["sessions"].items()
  )
  commits_valid = all(
    fields_match(
      commit,
...
def command_init(args):
  ...
  timestamp = now()
  ledger = {
    "schema_version": 4 if args.provider else 3,
    "slug": args.slug,
    "plan_path": str(plan_path),
    "repository": str(repository),
    "branch": args.branch,
...
def command_register_session(args):
  ...
  if ledger["schema_version"] != 4:
    required_model, required_effort = SPECIALISTS_BY_SCHEMA[ledger["schema_version"]][args.role]
    if (args.model, args.effort) != (required_model, required_effort):
      raise WorkflowError(f"{args.role} requires {required_model} at {required_effort}; fallback is forbidden")
  chat = {
  ...
  if ledger["schema_version"] == 4:
    ...
    if not configured_session_is_valid(chat):
      raise WorkflowError("Register the actual provider, model, effort and non-empty session ID")
  existing = ledger["sessions"].get(args.role)
  if existing is not None and existing != chat:
    if ledger["schema_version"] != 4 or existing["session_id"] != chat["session_id"]:
      raise WorkflowError(f"{args.role} already has a different registered chat")
    ledger.setdefault("model_changes", []).append({"at": now(), "role": args.role, "before": existing, "after": chat})
  if existing != chat:
    ledger["sessions"][args.role] = chat
    persist(args, ledger)
  print(json.dumps(chat, sort_keys=True))


def command_migrate_models(args):
  ledger = load_ledger(args.state)
  if ledger["schema_version"] not in (3, 4):
    raise WorkflowError("Model migration requires a six-specialist schema-v3 or schema-v4 workflow")
  try:
    selections = json.loads(args.selections)
  except (ValueError, TypeError) as error:
    raise WorkflowError("Invalid model selections") from error
  if (
    not isinstance(selections, dict)
    or set(selections) != set(SPECIALISTS_BY_SCHEMA[4])
    or any(not configured_session_is_valid(chat) for chat in selections.values())
    or any(role not in ledger["sessions"] or chat["session_id"] != ledger["sessions"][role]["session_id"] for role, chat in selections.items())
    or not args.reason.strip()
  ):
    raise WorkflowError("Migration must preserve all six session IDs and supply actual selections and a reason")
  if ledger["schema_version"] == 4 and ledger["sessions"] == selections:
    print(json.dumps(ledger, sort_keys=True))
    return
  ledger.setdefault("model_changes", []).append({
    "at": now(), "reason": args.reason, "previous_schema": ledger["schema_version"],
    "before": ledger["sessions"], "after": selections,
  })
  ledger["schema_version"] = 4
  ledger["sessions"] = selections
  persist(args, ledger)
  print(json.dumps(ledger, sort_keys=True))
...
def require_registered_specialists(ledger):
  invalid_roles = [
    ...
    if role not in ledger["sessions"] or not ledger["sessions"][role]["session_id"].strip()
  ]
  if invalid_roles:
...
def build_parser():
  ...
  initialize.add_argument("--provider", help="Use schema v4 for DSH-configured model selections")
  ...
  register_session.add_argument("--role", required=True, choices=sorted(SPECIALIST_ROLES))
  register_session.add_argument("--session-id", required=True)
  register_session.add_argument("--model", required=True)
  register_session.add_argument("--effort", required=True)
  register_session.add_argument("--provider")
  register_session.set_defaults(handler=command_register_session)

  migrate_models = subparsers.add_parser("migrate-models")
  migrate_models.add_argument("--selections", required=True)
  migrate_models.add_argument("--reason", required=True)
  migrate_models.set_defaults(handler=command_migrate_models)
