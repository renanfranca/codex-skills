...
class WorkflowStateCliTest(unittest.TestCase):
  ...
  def initialize_v2(self, root, slug="demo"):
    plan = root / f"{slug}.md"
    state = root / f"{slug}.dsh-workflow.json"
  ...
  def test_init_creates_a_complete_ledger(self):
    with tempfile.TemporaryDirectory() as directory:
      root = Path(directory)
      plan = root / "demo.md"
      state = root / "demo.dsh-workflow.json"
      plan.write_text("# Approved plan\n", encoding="utf-8")
  ...
  def test_show_rejects_a_corrupt_ledger_without_changing_it(self):
    with tempfile.TemporaryDirectory() as directory:
      state = Path(directory) / "demo.dsh-workflow.json"
  ...
  def test_show_reads_a_legacy_v1_ledger_without_rewriting_it(self):
    with tempfile.TemporaryDirectory() as directory:
      ...
      ledger = json.loads(state.read_text(encoding="utf-8"))
      ledger["schema_version"] = 1
      ledger["sessions"]["habit-curator"]["model"] = "deepseek/deepseek-v4.1-flash"
  ...
  def test_register_session_records_the_required_specialist_identity(self):
    with tempfile.TemporaryDirectory() as directory:
      root = Path(directory)
  ...
  def test_schema_v3_requires_exact_deepseek_high_before_implementation(self):
    with tempfile.TemporaryDirectory() as directory:
      root = Path(directory)
      ...
      for model, effort in (("wrong-model", "high"), ("deepseek/deepseek-v4.1-flash", "medium")):
