...
Create and register all six specialist tasks exactly once before the first `implementing` transition. This includes the `mutation-analyst` on `gpt-5.6-luna` at `xhigh`. Only the Coordinator communicates with specialists, grants assignments, and operates ledger leases. Reuse the registered tasks for the entire plan; never ask specialists to coordinate with each other.
...
New ledgers use schema v3 and capture both the declared base ref and its immutable `base_sha`. Existing schema-v1 and schema-v2 ledgers remain readable and retain their original transition, evidence, pull-request, and cleanup rules; reading them never migrates or rewrites them.
